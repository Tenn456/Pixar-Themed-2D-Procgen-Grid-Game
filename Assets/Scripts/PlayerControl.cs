...
public class PlayerControl : MonoBehaviour
{
    ...
    private void ProcessTileEvents()
    {
        ...
        {
            // Plays Ball Bouncing Animation (Removed camera shake and flash)
            Animator anim = currentTile.gameObject.GetComponent<Animator>();
        ...
        else if (currentTile.isGoal)
        {
            ...
            else
            {
                ...
                gm.cratesRemaining--;
                gm.cratesRemainingText.text = gm.cratesRemaining.ToString("00");
                gm.Invoke("RestartGame", goalReached.length);
            } 
        }
        else
        {
            StartCoroutine(WaitASecond(0.2f));
        }
    }

    IEnumerator WaitASecond(float d)
    {
