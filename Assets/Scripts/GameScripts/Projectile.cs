...
public class Projectile : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            StartCoroutine(DestroyWithTrail(GetComponent<TrailRenderer>()));
            //Destroy(gameObject);
        }     
    }

    IEnumerator DestroyWithTrail(TrailRenderer trail, float delay = 0.5f)
    {
        trail.emitting = false;
        yield return new WaitForSeconds(trail.time + delay);
        Destroy(gameObject);
    }
}
